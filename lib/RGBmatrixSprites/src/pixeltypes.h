...
// with 8-bit fraction
LIB8STATIC uint8_t lerp8by8( uint8_t a, uint8_t b, uint8_t frac)
{
     uint8_t result;
     if( b > a) {
         uint8_t delta = b - a;
...
LIB8STATIC uint16_t lerp16by16( uint16_t a, uint16_t b, uint16_t frac)
{
    uint16_t result;
    if( b > a ) {
        uint16_t delta = b - a;
...
struct CRGB {
    ...
    inline CRGB lerp8( CRGB & other, uint8_t frac)
    {
      CRGB ret;
    ...
    inline CRGB lerp16( CRGB & other, uint16_t frac)
    {
      CRGB ret;
